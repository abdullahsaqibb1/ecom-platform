...
export function ProductsPage() {
  ...
  const purgeMutation = useMutation({ mutationFn: (id: string) => apiRequest(ADMIN_API.productPermanent(id), { method: 'DELETE' }), onSuccess: async () => { await Promise.all([queryClient.invalidateQueries({ queryKey: ['products'] }), queryClient.invalidateQueries({ queryKey: ['inventory'] })]); setPurging(null); } });
  ...
  return <div>
    ...
    <ConfirmDialog isOpen={Boolean(purging)} title="Delete this product permanently?" description={`${purging?.name ?? 'This product'} and its configurations will be deleted. Orders referencing it must be deleted first.`} confirmLabel="Delete permanently" isLoading={purgeMutation.isPending} onClose={() => setPurging(null)} onConfirm={() => purging && purgeMutation.mutate(purging.id)} />
    {purgeMutation.error ? <div className="form-alert floating-alert">{purgeMutation.error.message}</div> : null}
  </div>;
}
