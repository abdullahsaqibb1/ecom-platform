  ...
  return (
    <>
      ...
      <div className={`mobile-menu ${menuOpen ? 'open' : ''}`}>
        <div className="mobile-menu-head"><span className="wordmark">{storeName}</span><button onClick={() => setMenuOpen(false)} aria-label="Close menu"><X /></button></div>
      ...
      <div className={`search-overlay ${searchOpen ? 'open' : ''}`}>
        ...
        <form onSubmit={submitSearch}><label htmlFor="site-search">What are you looking for?</label><div><input id="site-search" autoFocus={searchOpen} value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search earbuds, chargers, cables…" /><button type="submit" aria-label="Submit search"><Search /></button></div></form>
      </div>
    </>
  );
}
